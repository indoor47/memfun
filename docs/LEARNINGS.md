...
## 2026-05-06 — Parallel-agent epic CLOSED

### What shipped: 9 implementation PRs + 1 validation PR + 1 P0 follow-up across the parallel-agent epic
- **What**: Tracker #6 ("scale to 30-100 coordinating agents with worktrees + non-overlap decomposition") closes with all 9 child issues merged: #16 decomposition-time overlap detector (#9), #17 WorktreeManager (#8), #19 `memfun doctor` (#7), #20 wizard multi-provider + sonnet-4-6 default (#15), #21 per-instance AgentManager (#12), #22 bounded concurrency in `fan_out` (#10), #23 hierarchical decomposer (#11), #24 project-root scoped filesystem ops (#13), #25 Experiment A validation test, #26 P0 follow-up cleanup-gate (#18). Test count: 597 → 854 (+257). 5/5 ruff clean, pyright strict on packages, pip-audit transitive-only (no new advisories from the epic). Only #14 (P6 stretch — MergeCoordinator three-way merge driver) remains open by design.
- **Why it matters**: The load-bearing architectural learning is **preventing overlap at decomposition time is structurally simpler than merging after**. PR #9's overlap detector + `adecompose`'s singleton-group split takes the merge-conflict surface from "6/10 pairwise conflicts in Experiment A" to **structurally zero** through `WorkflowEngine`. Worktrees (#8) provide isolation, the cleanup gate (#18) closes the destructive-API hole that isolation would otherwise inherit, and per-instance keying (#12) + bounded concurrency (#10) make the runtime fan-out actually scale to N=30+. Hierarchical decomposition (#11) gives N=100 a recursion path. The whole epic is the answer to "can a single repo coordinate 100 agents safely" — and the answer is yes, *if* you reject overlap at the planning step instead of trying to be clever with `git merge-tree`.
- **Source**: PRs #16-#26, validation `tests/test_validation_experiment_a.py`, reliability sweep 2026-05-06.

### Validation pattern: Test A / Test B / Test C-skipped closes the loop on a hypothesis-driven architectural change
- **What**: PR #25 introduced a three-test harness for closing-the-loop on the overlap-detector hypothesis: **Test A** = property assertion at the decomposer layer (overlap groups are split into singletons), **Test B** = property assertion at the orchestrator layer (no two `fan_out` invocations get overlapping write-targets in the same group), **Test C** = end-to-end clean merge-back of N worktrees, *intentionally `pytest.skip`-ed* with a reason string pointing to the missing `WorkflowEngine` merge-back step. Test C as a skip is documentation: it tells the next reader "the property would hold here, but the code that produces the property doesn't exist yet — see #14".
- **Why it matters**: When closing the loop on a hypothesis-driven architectural change, write the test for the property you *don't* yet have implementation for. The skip is a load-bearing signal — it both prevents future regressions (the skip will fail loudly if the test is ever un-skipped without the feature) and makes the gap in the architecture explicit in CI.
- **Source**: `tests/test_validation_experiment_a.py` (PR #25), 2026-05-06.

### Long Opus 4.7 implementer runs ABANDON ambiguously — happened twice (#13, #18). Recovery is mechanical: branch on origin or uncommitted in worktree → commit + push + PR
- **What**: This pattern surfaced for the second time in this epic. The implementer agent for issue #18 (cleanup gate) — like #13 before it — emitted a final natural-language message that wasn't a `gh pr create` confirmation, and exited. The work was complete on the worktree branch. Recovery: `gh pr list --head <branch>` (empty) → `git -C <wt> status` (uncommitted) → `git -C <wt> add -A && git -C <wt> commit && git -C <wt> push -u origin HEAD && gh pr create`. The pattern is now reliable enough that a small wrapper script is justified.
- **Why it matters**: After dispatching ANY long-running implementer agent, the post-condition to check is not "did the agent say it succeeded" but "is the branch on origin AND does `gh pr list --head <branch>` return one row". If either check fails, the recovery is mechanical and 100% successful — the work is on disk, just not surfaced. Two occurrences in one epic graduates this from "anomaly" to "expected failure mode" and warrants tooling.
- **Source**: Issue #18 implementer recovery → PR #26 (and earlier #13 → PR #24), 2026-05-06.

### Local `git log` lies after a `gh pr merge` from another machine — `git fetch origin main` before any reliability sweep
- **What**: Reliability sweep initially flagged "cleanup gate not on main" because `git log --oneline -30` from local main showed `c36a638` (test count bump) as HEAD, missing PR #26's merge commit `aaa6759`. The PR had been merged via `gh pr merge` on origin and the local clone hadn't been fetched since. `git fetch && git pull --ff-only` corrected the view immediately.
- **Why it matters**: A reliability monitor that runs from a stale local checkout produces false-positive drift findings. First action of any sweep: `git fetch origin main && git status -sb`. If `behind` count > 0, fast-forward before reading any code. This is implicit when CI runs in a fresh checkout but easy to miss in local invocations.
- **Source**: Reliability sweep 2026-05-06, initial false negative on `_check_in_managed_root` grep.
